...
for pkg in $PACKAGES; do
    ...
    if [ "$pkg" == "aws-deepracer-core" ]; then
        ...
        if [ "$ROS_DISTRO" == "humble" ]; then
            PACKAGE_DEPS="$PACKAGE_DEPS, ros-$ROS_DISTRO-rplidar-ros, ros-$ROS_DISTRO-libcamera, ros-$ROS_DISTRO-web-video-server, ros-$ROS_DISTRO-rosbag2, ros-$ROS_DISTRO-rosbag2-py, ros-$ROS_DISTRO-rosbag2-storage-mcap"
        fi
