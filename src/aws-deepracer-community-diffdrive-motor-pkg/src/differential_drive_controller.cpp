...
MotorSpeeds DifferentialDriveController::convertServoToMotorSpeeds(float angle, float throttle)
{
  ...
  if (angle > 0) {
      // Turning right: left wheel is outer, right wheel is inner
    float max_differential = config_.max_right_differential;
    left_speed = throttle * max_speed * (1.0f - config_.center_offset);   // Outer wheel at commanded speed
    right_speed = throttle * max_speed * (1.0f + config_.center_offset) *
      (1.0f - max_differential * std::abs(angle));               // Inner wheel reduced
  } else if (angle < 0) {
      // Turning left: right wheel is outer, left wheel is inner
    float max_differential = config_.max_left_differential;
    right_speed = throttle * max_speed * (1.0f - config_.center_offset);   // Outer wheel at commanded speed
    left_speed = throttle * max_speed * (1.0f + config_.center_offset) *
      (1.0f - max_differential * std::abs(angle));              // Inner wheel reduced
  } else {
      // Going straight: both wheels at same speed
    left_speed = throttle * max_speed * (1.0f - config_.center_offset);
    right_speed = throttle * max_speed * (1.0f + config_.center_offset);
  }
